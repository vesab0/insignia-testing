...
from backend.services import ReservationService


def next_open_date(days_ahead=7):
    """Return a future date that is within booking limits and not Monday."""
    reservation_date = datetime.now() + timedelta(days=days_ahead)
    while reservation_date.weekday() == 0:
        reservation_date += timedelta(days=1)
    return reservation_date.strftime("%Y-%m-%d")
...
class TestAPIEndpoints:
    ...
    def test_create_reservation_success(self, client):
        ...
        response = client.post(
            "/api/reservations", json=data, content_type="application/json"
        )
    ...
    def test_newsletter_subscription(self, client):
        ...
        response = client.post(
            "/api/newsletter", json=data, content_type="application/json"
        )
        assert response.status_code == 201
        result = response.get_json()
        assert result["success"] is True
