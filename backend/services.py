...
class EmailService:
    ...
    def send_reservation_confirmation(self, reservation: Reservation) -> bool:
        ...
        try:
            ...
            self._generate_confirmation_email(reservation)
        ...
        try:
            ...
            if existing:
                if existing.active:
                    ...
                    logger.info(
                        f"Newsletter subscription reactivated: {newsletter.email}"
                    )
                    return {
                        "status": "reactivated",
    ...
    def _generate_confirmation_email(self, reservation: Reservation) -> str:
        ...
        return dedent(
            ...
            We look forward to serving you at La Maison Elegante.
...
class MenuService:
    ...
    def get_menu(self, category: Optional[str] = None) -> List[Dict]:
        ...
        try:
            if category:
                return [
                    item for item in self.menu_items if item["category"] == category
                ]
