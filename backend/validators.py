...
class DateValidator(BaseValidator):
    ...
    def __init__(self, data: str, min_advance_hours: int = 2, max_advance_days: int = 90):
        super().__init__(data)
    ...
    def is_valid(self) -> bool:
        ...
        try:
            ...
            if reservation_date < min_date:
                self.add_error(f"Reservations must be made at least {self.min_advance_hours} hours in advance")
                return False
            
            # Check maximum advance booking
            max_date = today + timedelta(days=self.max_advance_days)
            if reservation_date > max_date:
                self.add_error(f"Reservations can only be made up to {self.max_advance_days} days in advance")
                return False
...
class TimeValidator(BaseValidator):
    ...
    def is_valid(self) -> bool:
        ...
        if self.data not in self.VALID_TIMES:
            self.add_error(f"Invalid time. Available times: {', '.join(self.VALID_TIMES)}")
            return False
    ...
    def is_valid(self) -> bool:
        ...
        try:
            ...
            if guests > self.max_guests:
                self.add_error(f"Maximum {self.max_guests} guests per reservation. Please contact us for larger parties.")
                return False
